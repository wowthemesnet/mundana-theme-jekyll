...
1. Define class Optimier
	1. Init - params, lr
		1. Why do we add list of parameters at learning rate?
	1. step
	1. zero_grad
...
2. make dataset object 1) get the length and 2) get items using index
	1. compare those with originals'
...
3. make `def fit()` which gets data from dataloader class. ***(6th training)**1. 
...
3. Make `Sampler` class [^7]
	1. initialized with 4 params, (int) dataset len, (int) bs, and (bool) shuffle
